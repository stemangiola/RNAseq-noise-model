...
counts_stan = 
  ...
  #filter(symbol%in%c("A2M-AS1", "A1BG")) %>%
...
data_for_stan_MPI = list(
  ...
  D = counts_stan_MPI %>% distinct(`Data base`) %>% nrow(),
  ...
  X = counts %>% 
    distinct(sample, `Data base`) %>% 
    mutate(
      `Data base` = `Data base` %>% factor(levels=
         counts %>% 
         distinct(sample, `Data base`) %>% 
         count(`Data base`) %>% 
         arrange(n %>% desc) %>% 
...
#save(fit_MPI, file="fit_MPI_05032019.RData")
Sys.time()

#fit_parsed = fit_MPI %>% spread_draws(lambda[G], sigma[G], sigma_raw[G], sigma_raw_gen[G], lambda_gen[G])

fit_parsed = fit_MPI %>% summary() %$% summary %>% as_tibble(rownames="par") %>% filter(grepl("lambda|sigma", par)) %>% separate(par, c("par", "G"), sep="\\[|\\]") %>% select(par, G, "50%") %>% spread(par, "50%") %>% mutate(G = G %>% as.integer)
...
fit_parsed %>%
  ...
  left_join(data_for_stan$symbols %>% mutate(G=1:n()) ) %>% left_join(
    fit_MPI_parsed %>%
    mean_qi() %>%
    left_join(data_for_stan_MPI$symbols %>% gather(idx_MPI, symbol) %>%
                drop_na() %>%
                arrange(idx_MPI, symbol) %>% mutate(G=1:n()) ) ,
    by = c("symbol")
  ) %>%
{ggplot((.),  aes(x = lambda.x, y = lambda.y , label=symbol , idx_MPI=idx_MPI, color=symbol=="AKR1C3")  ) + geom_point()} %>% plotly::ggplotly()
    ...
    left_join(data_for_stan_MPI$symbols %>% gather(idx_MPI, symbol) %>%
                drop_na() %>%
                mutate(G=1:n()) )  %>%
      left_join(counts_stan_MPI %>% select(symbol, `read count normalised`) %>% group_by(symbol) %>% summarise(`read count normalised` = `read count normalised` %>% `+` (1) %>% log %>% mean)) %>%
{ggplot((.),  aes(x = lambda, y = `read count normalised` , label=symbol , idx_MPI=idx_MPI, color=symbol=="AKR1C3")  ) + geom_point()} %>% plotly::ggplotly()
...
fit_parsed %>% 
    ...
    left_join(
      rstan::extract(fit_MPI, "sigma_raw_gen") %>% as.data.frame %>% as_tibble() %>% head(n=1) %>% gather(G, gen_quant) %>% mutate(G=1:n())
    ) %>%
    ...
    ggplot(aes(x=lambda, y=my_sigma, lable=symbol, color=is_real)) + 
    geom_point(alpha=0.5) +
    stat_function(
        fun = function(x) {
            slope =  fit_MPI %>% rstan::extract(pars="sigma_slope") %>% as.data.frame %>% as_tibble %>% pull(1) %>% median
            intercept = fit_MPI %>% rstan::extract(pars="sigma_intercept") %$% sigma_intercept %>% median()
            (slope * x + intercept) ;
        },
        geom="line"
    ) +
  # geom_abline(slope = -0.35, intercept = 0.50) +
  #stat_smooth( method="lm", formula= (y ~ exp(x)) ) +
  scale_y_continuous(trans="log") + my_theme
		
fit_parsed %>% 
  #median_qi() %>%   
  left_join(
      rstan::extract(fit_MPI, "lambda_gen") %>% as.data.frame %>% as_tibble() %>% head(n=1) %>% gather(G, gen_quant) %>% mutate(G=1:n())
    ) %>%  
  gather(is_real, my_lambda,  c("lambda", "gen_quant")) %>%
  ggplot(aes(my_lambda, color=is_real)) +
  geom_freqpoly() + my_theme
...
fit_predicted = 
  ...
  left_join(
    counts_stan %>% select(symbol, sample, `read count normalised`, `read count`)
  ) %>% 
  do_parallel_start(40, "symbol") %>%
 	do({
  ...
  mutate(`error scaled` =  ((`read count normalised` - predicted_NB) / (`read count normalised` + 1) ) %>%  abs) %>%
  left_join(
    (.) %>%
    group_by(symbol) %>% 
...
fit_predicted %>% 
select(sample, symbol, `read count`, `read count normalised`, `gene error mean`) %>%
write_csv("t_cells_with_error_qq_plot_means.csv")
...
fit_predicted %>%
    ...
    summarise(`error mean` = `error scaled` %>% mean) %>%
    ungroup() %>%
...
fit_parsed_TCGA = fit_MPI_TCGA %>% summary() %$% summary %>% as_tibble(rownames="par") %>% filter(grepl("lambda|sigma", par)) %>% separate(par, c("par", "G"), sep="\\[|\\]") %>% select(par, G, "50%") %>% spread(par, "50%") %>% mutate(G = G %>% as.integer)

fit_parsed_TCGA %>% 
    #median_qi() %>% 
    
    left_join(
      rstan::extract(fit_MPI_TCGA, "sigma_raw_gen") %>% as.data.frame %>% as_tibble() %>% head(n=1) %>% gather(G, gen_quant) %>% mutate(G=1:n())
    ) %>%
    
    # left_join(
    #   data_for_stan_MPI$ens_iso %>% 
    #     gather(idx_MPI, symbol) %>%
    #   drop_na() %>%
    #   arrange(idx_MPI, symbol) %>% mutate(G=1:n()) 
    # ) %>%
    gather(is_real, my_sigma,  c("sigma_raw", "gen_quant")) %>%
    ggplot(aes(x=lambda, y=my_sigma,  color=is_real)) + 
    geom_point(alpha=0.5) +
    stat_function(
        fun = function(x) {
            slope =  fit_MPI %>% rstan::extract(pars="sigma_slope") %>% as.data.frame %>% as_tibble %>% pull(1) %>% median
