...
data_for_stan = counts_stan %>%
  ...
  counts = (.) %>% select(`read count normalised`) %>% pull(1),
  sample_idx = 
    (.) %>%
    pull(sample_idx),
  # Info on data set
...
counts_stan_MPI =
  ...
  do(
    ...
    left_join(
      (.) %>% 
      distinct(idx_MPI, sample, symbol, `read count normalised`) %>% 
      arrange(idx_MPI, symbol) %>%
...
data_for_stan_MPI = list(
  ...
  select(-i) %>%
    ...
    symbols = foreach(idx = counts_stan_MPI %>% distinct(idx_MPI) %>% pull(idx_MPI), .combine = full_join) %do% {
      counts_stan_MPI %>%
          filter(idx_MPI == idx) %>%
          distinct(symbol) %>% 
          rbind(  counts_stan_MPI %>% filter(idx_MPI == idx) %>% pull(end) %>% max) %>%
...
fit_parsed %>%
  ...
  left_join(data_for_stan$symbols %>% mutate(G=1:n()) ) %>% left_join(
    fit_MPI_parsed %>%
    mean_qi() %>%
    left_join(data_for_stan_MPI$symbols %>% gather(idx_MPI, symbol) %>% mutate(G=1:n()) ) ,
    by = c("symbol")
  ) %>%
ggplot(  aes(x = lambda.x, y = lambda.y  )  ) + geom_point()
