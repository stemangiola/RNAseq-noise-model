...
xx = data_for_stan_MPI %>%
  left_join(
    (.) %>% group_by(symbol) %>% summarise(s=`read count normalised` %>% sum) %>% arrange(s)
  ) %>%
  filter(s > 0) %>%
 group_by(symbol) %>%
   do({
  ...
  mutate(`error scaled` =  ((`read count normalised` - predicted) / (`read count normalised` + 1) ) %>%  abs) %>%
  left_join(
    (.) %>%
