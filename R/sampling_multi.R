...
sampling_multi <- function(models, data, map_fun = sampling_multi_noop, combine_fun = sflist2stanfit, chains = 4, cores = parallel::detectCores(),
  ...
  fit_fun <- function(i) {
    ...
    if(!is.null(cache_dir)) {
      ...
      if(file.exists(filename)) {
        ...
        if(!is.list(cached_data)) {
          ...
          if(is.null(cached_data$data) || is.null(fit_from_cache) || is.null(cached_data$model_code)) {
            not_cached_msg <- "Does not contain fit or data or code"
            cached <-  FALSE
          } else if(class(fit_from_cache) != "stanfit") {
            not_cached_msg <- "Not a stanfit"
            cached <-  FALSE
          } else if(model_code != cached_data$model_code) {
    ...
    if(!cached) {
      ...
      if(!is.null(cache_dir)) {
        saveRDS(list(data = arg_list$data, fit = single_fit, model_code = model_code), filename)
      }
    }
  ...
  if(cores > 1) {
    ...
    results_raw <- lapply(X = ids_to_compute, FUN = fit_fun)
  }
...
normalize_model_code <- function(code) {
