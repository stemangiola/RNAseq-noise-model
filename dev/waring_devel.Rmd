...
test_model <- cmdstanr::cmdstan_model(here("stan", "waring_simple_test.stan"))
```

```{r}
generator <- function(N, r_sd = 5, r_known = FALSE) {
  function() {
    mu <- rlnorm(1, 3, 1)
    ...
    list( true = list(mu = mu, phi = phi, r = r), 
          observed = list(N = N, y = rgwaring4(N, mu, phi, r), r_sd = r_sd, r_known = r_known, r_true = r_true))
  }
}

test_data <- generator(30, r_known = TRUE)()

cmdstan_fit <- test_model$sample(data = test_data$observed)
fit <- rstan::read_stan_csv(cmdstan_fit$output_files())

xx <- (rstan::extract(fit))$r

evaluate_all_params(rstan::extract(fit), test_data$true)
```
...
sbc_res <- sbc(test_model, generator(100, r_sd = 5, r_known = TRUE), N_steps = 1, cache_dir = cache_dir, cores = 1)
#sbc_res <- sbc(test_model, generator(100, r_sd = 5), N_steps = 30)
...
load_sbc_fit <- function(run) {
  ...
  # fit
  readRDS(here("local_temp_data","sbc_waring",sprintf("%06d.rds",run)))$fit
}
