...
logNormal_multinomial_fit = sampling_check_return(logNormal_multinomial_model, control =list(max_treedepth =12)) 
logStudent_multinomial_fit = sampling_check_return(logStudent_multinomial_model, control =list(max_treedepth =13)) 
...
negBinomial_gen =
  ...
  left_join(
    tibble(N = 1:( counts_tidy %>% pull(sample) %>% levels %>% length ))
  )
