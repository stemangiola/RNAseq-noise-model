...
fit1 =
  sampling_check_return(
     stan_model(here("stan",sprintf("%s.stan", "dirichlet_multinomial"))),
    data = data_for_stan %>% lplyr::mutate(is_prior_asymetric = 1),
    control = list( max_treedepth = 10)
)
fit2 =
  sampling_check_return(
     stan_model(here("stan",sprintf("%s.stan", "dirichlet_multinomial_explicit"))),
    data = data_for_stan %>% lplyr::mutate(is_prior_asymetric = 1),
...
fit1 %>% 
  ...
  geom_point() + my_theme + xlab("Analytical") + ylab("Explicit")
```
