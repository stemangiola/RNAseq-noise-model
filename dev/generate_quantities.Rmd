...
fits =
  ...
  foreach(is_prior_asymetric = c(1), .combine = c) %:%
  foreach(variance_size = c(1,G), .combine = c) %do% {

    # If not applicable skip
    if(model_name %in% c("multinomial", "dirichlet_multinomial") & variance_size > 1) return(NULL)

    # Trick for naming list
    my_list = list()
    my_list[[sprintf("%s_a-%s_v-%s", model_name, is_prior_asymetric, variance_size)]] =
      sampling_check_return(
        model,
        data =
          data_for_stan %>%
          lplyr::mutate(is_prior_asymetric = is_prior_asymetric) %>%
          lplyr::mutate(variance_size = variance_size),
        control = list(
          max_treedepth = switch(
            model_name,
            "logNormal_multinomial" = 12,
...
# Fit a single model
fits$`negBinomial_a-1_v-1000_2` =
  sampling_check_return(
    stan_model(here("stan",sprintf("%s.stan", "negBinomial"))),
    data =
      data_for_stan  %>%
      lplyr::mutate(is_prior_asymetric = 1) %>%
      lplyr::mutate(variance_size = G),
    control = list( max_treedepth = 12)
)
fits$`logNormal_multinomial_a-1_v-1_2` =
  sampling_check_return(
    stan_model(here("stan",sprintf("%s.stan", "logNormal_multinomial"))),
    data =
      data_for_stan  %>%
      lplyr::mutate(is_prior_asymetric = 1) %>%
      lplyr::mutate(variance_size = 1),
    control = list( max_treedepth = 12)
)
```
...
Plots variances trend

```{r}

# Get generated quantities
foreach(
  fit = fits[fits %>% names %>% grep("v-1", ., invert=T)],
  model_name = fits %>% names %>% grep("v-1", ., invert=T, value = T),
  .combine = bind_rows
) %do% {
  library(dplyr) #On Windows, %dopar%-ed code does not share the main session
  library(tidybayes) #On Windows, %dopar%-ed code does not share the main session
  fit %>%
    spread_samples(sigma_raw[G], lambda[G]) %>%
    ungroup() %>%
    mutate(model_name)
} %>%
  ...
  {
    (.) %>%
    ggplot(aes(x = lambda, y = sigma_raw)) +
    stat_ellipse(geom = "polygon", alpha = 0.2, color=NA, aes(group=G)) +
  ...
  # Save plot
  {
    ggsave(plot = (.), here("dev", "variance_trend.png"), width=18.3, height=18.3, units = "cm")
    (.)
  }
